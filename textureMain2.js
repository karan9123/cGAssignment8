...
function setUpTextures(){
    ...
    baseBallTexture = gl.createTexture();
    
    let baseImage = new Image();
    baseImage.src = 'baseball.jpg';

    baseImage.onload = () => {
        doLoad (baseBallTexture, baseImage);
    };
}
...
function drawCurrentShape () {
    ...
    if (curTexture == "globe") {
      gl.uniform1i(program.uBackScene,1)
      gl.activeTexture(gl.TEXTURE0);
      gl.bindTexture(gl.TEXTURE_2D, worldTexture);
      gl.uniform1i(program.uTheTexture, 0);
  }
  else if(curTexture=="myimage"){
    gl.uniform1i(program.uBackScene, 0);
      gl.activeTexture(gl.TEXTURE0+1);    
      gl.bindTexture(gl.TEXTURE_2D, baseBallTexture);
      gl.uniform1i(program.ubaseBallTexture, 1);
  }
  else {
    gl.uniform1i(program.uBackScene, 2);
  }
...
function initProgram (vertexid, fragmentid) {
  ...
  program.ubaseBallTexture = gl.getUniformLocation (program, 'baseBallTexture');
  program.uBackScene = gl.getUniformLocation (program, 'backScene');
